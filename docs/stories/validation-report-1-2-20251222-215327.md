...
1. **与架构文档的 api-config 命名规划存在不一致风险**
   - Evidence: `docs/architecture.md` 的前端结构示例规划了 `features/api-config/ApiConfigPanel.tsx`、`hooks/useApiConfig.ts`（以及对应导出结构）；但当前 Story 规划为 `DifyCredentialForm.tsx`、`useDifyCredentialForm.ts`（Story Tasks L40-52, L155-172）。
...
3. **仓库内不存在 `**/project-context.md`（本次对齐应明确以哪些文档为准）**
   ...
   - Recommendation: 在 Story Dev Notes 明确“权威来源”为 `docs/architecture.md`、`docs/epics.md`、`docs/prd.md`，并指出没有 project-context。
