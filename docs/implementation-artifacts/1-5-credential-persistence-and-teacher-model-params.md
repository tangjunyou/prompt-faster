...
- [x] **Task 4: 仓储层（Repository）与查询约束** (AC: #1, #2)
  ...
  - [x] 4.3 保持"Repository 是唯一数据库访问点"的边界（参考 `docs/project-planning-artifacts/architecture.md#数据边界`）
...
- [x] **Task 8: 新增保存/加载配置的 API 服务与 TanStack Query Hooks** (AC: #1, #2)
  ...
  - [x] 8.3 仍遵循架构约束：`services/*` 只导出纯函数；hooks 封装 TanStack Query（参考 `docs/project-planning-artifacts/architecture.md#TanStack Query 使用约束`）
...
- **安全与加密约束**：API Key 必须 AES-GCM 加密存储 (NFR9)，且密钥仅存于内存，不得持久化（见 `docs/project-planning-artifacts/architecture.md#Authentication & Security`）
- **数据边界**：
...
- [Source: docs/project-planning-artifacts/epics.md#Story-1.5] - 验收标准原文
- [Source: docs/implementation-artifacts/sprint-status.yaml#development_status] - story_key 与状态流转
- [Source: docs/project-planning-artifacts/architecture.md#Authentication-&-Security] - AES-GCM + Argon2 + 密钥仅内存
- [Source: docs/project-planning-artifacts/architecture.md#数据边界] - API Key Manager/Repository 唯一访问点
...
**Sprint 跟踪文件:**
- `docs/implementation-artifacts/sprint-status.yaml` — 修改，同步 story 状态
